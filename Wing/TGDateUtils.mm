...
static char value_common_separator = ' ';
...
+ (NSString *)stringForMessageListDateNew:(int)date
{
    time_t t = date;
    struct tm timeinfo;
    localtime_r(&t, &timeinfo);
    ...
    NSString *strMonth = [self getMonth:timeinfo.tm_mon+1];

    
     return [[NSString alloc] initWithFormat:@"%d%c%@%c%02d %@ %d:%02d", timeinfo.tm_mday, value_common_separator, strMonth, value_common_separator, timeinfo.tm_year - 100,@"at", (timeinfo.tm_hour - 12 == 0) ? 12 : timeinfo.tm_hour, timeinfo.tm_min];
...
+ (NSString *)stringForRelativeLastSeen:(int)date
{
    ...
    {
        NSInteger dayDiff = timeinfo.tm_yday - timeinfo_now.tm_yday;
        
//        NSInteger minutesDiff = (t_now - date) / 60;
        NSInteger hoursDiff = (t_now - date) / (60 * 60);
        
        if (dayDiff == 0 && hoursDiff <= 23)
        {
            
            return [[NSString alloc] initWithFormat:@"%@ %@ %d:%02d",@"today", @"at", timeinfo.tm_hour == 0 ? 12 : timeinfo.tm_hour, (timeinfo.tm_min)];
            //raj
            /*if (minutesDiff < 1)
            {
                return @"just now";
            ...
            {
                return [[NSString alloc] initWithFormat:@"%ld %s ago", (long)minutesDiff, minutesDiff == 1 ? "minute" : "minutes"];
            }
            else
            {
                return [[NSString alloc] initWithFormat:@"%ld %s ago", (long)hoursDiff, hoursDiff == 1 ? "hour" : "hours"];
            }*/
        }
        else if (dayDiff == 0 || dayDiff == -1)
        {
            ...
            {
                if (timeinfo.tm_hour < 12)
                    return [[NSString alloc] initWithFormat:@"%@%s%@ %d:%02d", dayDiff == 0 ? @"" : @"yesterday", dayDiff == 0 ? "" : " ", @"at", timeinfo.tm_hour == 0 ? 12 : timeinfo.tm_hour, timeinfo.tm_min];
                else
                    return [[NSString alloc] initWithFormat:@"%@%s%@ %d:%02d", dayDiff == 0 ? @"" : @"yesterday", dayDiff == 0 ? "" : " ", @"at", (timeinfo.tm_hour - 12 == 0) ? 12 : timeinfo.tm_hour, timeinfo.tm_min];
            }
            else
            {
                return [[NSString alloc] initWithFormat:@"%@%s%@ %02d:%02d", dayDiff == 0 ? @"" : @"yesterday", dayDiff == 0 ? "" : " ", @"at", timeinfo.tm_hour, timeinfo.tm_min];
            }
        }
        else
        {
...
/*+ (NSString *)stringForRelativeLastSeen:(int)date
{
    ...
    // NSLog(@"%@",[self stringForDayOfWeek:date]);
    ...
    // NSLog(@"%@",weekdayNameFull(timeinfo.tm_wday));
    ...
    else
    {
        ...
        else if (dayDiff == 0 || dayDiff == -1)
        {
            if (dateHas12hFormat())
            {
                ...
                // return [[NSString alloc] initWithFormat:@"%@%s%@ %d:%02d", dayDiff == 0 ? @"" : value_yesterday, dayDiff == 0 ? "" : " ", value_at, (timeinfo.tm_hour - 12 == 0) ? 12 : (timeinfo.tm_hour - 12), timeinfo.tm_min];
            }
            else
            {
                return [[NSString alloc] initWithFormat:@"%@%s%@ %02d:%02d", dayDiff == 0 ? @"" : @"yesterday", dayDiff == 0 ? "" : " ", @"at", timeinfo.tm_hour, timeinfo.tm_min];
        ...
        else
        {
            ...
            //                return [[NSString alloc] initWithFormat:@"%d%c%d%c%02d", timeinfo.tm_mon + 1, value_date_separator, timeinfo.tm_mday, value_date_separator, timeinfo.tm_year - 100];
            //            else
            //  return [[NSString alloc] initWithFormat:@"%@%s%@ %d:%02d", dayDiff == 0 ? @"" : value_yesterday, dayDiff == 0 ? "" : " ", value_at, (timeinfo.tm_hour - 12 == 0) ? 12 : timeinfo.tm_hour, timeinfo.tm_min];
