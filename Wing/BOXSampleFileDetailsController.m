...
- (void)viewDidLoad 
{
    ...
    UIBarButtonItem *uploadButton = [[UIBarButtonItem alloc] initWithTitle:@"Send" style:UIBarButtonItemStylePlain target:self action:@selector(sendAction:)];
...
- (void)downloadCurrentItem
{
    ...
    [request performRequestWithProgress:^(long long totalBytesTransferred, long long totalBytesExpectedToTransfer) {
        ...
        if (error == nil) {
            ...
            [delegate BoxfileDownloaded:finalPath withExtension:((BOXFile *)self.item).extension];
...
//            [alertView show];
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"Download error" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
            [alertView show];
