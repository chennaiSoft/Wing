                                   ...
                                   if (!connectionError) {
                                       ...
                                       dispatch_async(dispatch_get_main_queue(), ^(void){
                                           if (jsonData.count) {
                                               ...
                                               [self updateImage:thumbURL with:URL with:jsonData successBlock:^(UIImage *image, NSUInteger videoDuration, NSError *error) {
...
- (void)updateImage:(NSString*)thumbURL with:(NSString*)URL  with:(NSDictionary*)jsonData successBlock:(void (^)(UIImage *image,NSUInteger videoDuration,NSError *error))succeedBlock{
    
    [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:[NSURL URLWithString:thumbURL] options:SDWebImageDownloaderContinueInBackground progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished) {
                                       ...
                                       dispatch_async(dispatch_get_main_queue(), ^(void){
                                           if (jsonData.count) {
                                               ...
                                               if ([jsonData firstObject][quality]) {
                                                   ...
                                                   [self updateImage:jsonData[0][quality] with:vimdeoURLString with:jsonData[0] successBlock:^(UIImage *image, NSUInteger videoDuration, NSError *error) {
